...
我们可以把每一头牛看作重量为 $$\ w_i \$$ ，价值为 $$\ (t_i - x w_i) \$$ 的一件物品，我们要求的就是在**总容量为W的情况下能容纳物品的最大价值**。事实上道题还要复杂一点，因为题目要求的是**所有奶牛的总重量不小于** $W$ ，而不是总容量为W，我们需要把所有**重量大于W的情况看作重量为W来处理**。最后我们**判断在容量大于等于W时价值的总和是否大于等于0，大于等于0成立，否则不成立**，之后使用二分继续搜索。代码如下
```cpp
#include <bits/stdc++.h>
using namespace std;
